...
import {
  ...
  Grid2X2, Layers3, Lightbulb, ListFilter, Menu, Network, Play, RotateCcw, Search,
  Sparkles, Target, Timer, Trophy, X, Zap,
} from 'lucide-react';
import { ErrorBoundary } from '@/components/error-boundary';
import { Toaster } from '@/components/ui/toaster';
import { TooltipProvider } from '@/components/ui/tooltip';
import { patterns, problems, type Problem, type ProblemStatus } from '@/data/problems';
...
function Lesson({ progress, setStatus, toggleBookmark }: { progress: Persisted; setStatus: (slug: string, status: ProblemStatus) => void; toggleBookmark: (slug: string) => void }) {
  ...
  const chooseAnswer = (index: number) => setAnswer(index);
  const next = () => { if (step < problem.steps.length - 1) setStep((value) => value + 1); else setStatus(problem.slug, 'completed'); };
  return <main className="lesson-shell">
    ...
    <div className="lesson-layout">
      ...
      <section className="lesson-main">
        ...
        {isDone ? <div className="complete-card"><div className="complete-icon"><Trophy size={31} /></div><h2>Pattern added to your toolkit.</h2><p>You completed this guided lesson. The next time this shape appears, you’ll have a place to start.</p><div style={{ display: 'flex', justifyContent: 'center', gap: 10, flexWrap: 'wrap' }}><button className="btn btn-secondary" onClick={() => { setStep(0); setStatus(problem.slug, 'in-progress'); }} data-testid="button-review-lesson"><RotateCcw size={14} /> Review lesson</button><Link href="/problems" className="btn btn-primary" data-testid="button-next-problem">Choose another <ArrowRight size={14} /></Link></div></div> : <LessonStepContent problem={problem} step={step} answer={answer} setAnswer={chooseAnswer} showHints={showHints} setShowHints={setShowHints} codeTab={codeTab} setCodeTab={setCodeTab} />}
        {!isDone && <div className="lesson-actions"><button className="btn btn-secondary" disabled={step === 0} style={{ opacity: step === 0 ? .45 : 1 }} onClick={() => setStep((value) => value - 1)} data-testid="button-previous-step"><ArrowLeft size={14} /> Previous</button><button className="btn btn-primary" disabled={step === 2 && answer === null} onClick={next} data-testid="button-next-step">{step === problem.steps.length - 1 ? 'Complete lesson' : 'Next step'} <ArrowRight size={14} /></button></div>}
      </section>
...
function LessonStepContent({ problem, step, answer, setAnswer, showHints, setShowHints, codeTab, setCodeTab }: { problem: Problem; step: number; answer: number | null; setAnswer: (value: number) => void; showHints: number[]; setShowHints: (value: number[] | ((items: number[]) => number[])) => void; codeTab: 'starter' | 'solution'; setCodeTab: (value: 'starter' | 'solution') => void }) {
  ...
  if (item.kind === 'checkpoint') return <div className="panel lesson-card"><StepHeading icon={<CircleHelp />} item={item} /><div className="checkpoint"><h3>{problem.checkpoint.question}</h3><p>Choose an answer to unlock the next step. Getting it wrong is useful information, not a penalty.</p><div className="choice-list">{problem.checkpoint.choices.map((choice, index) => { const checked = answer !== null; const result = checked ? (index === problem.checkpoint.answer ? 'correct' : index === answer ? 'wrong' : '') : ''; return <button className={`choice ${answer === index ? 'selected' : ''} ${result}`} onClick={() => setAnswer(index)} key={choice} data-testid={`button-choice-${index}`}><span className="choice-dot">{result === 'correct' ? <Check size={11} /> : result === 'wrong' ? <X size={11} /> : null}</span>{choice}</button>; })}</div>{answer !== null && <div className="checkpoint-result" style={{ color: answer === problem.checkpoint.answer ? '#28673a' : '#954e40' }}>{answer === problem.checkpoint.answer ? 'Exactly. You spotted the invariant.' : 'Not quite. Re-read the pattern signal, then try again.'}</div>}</div></div>;
  return <div className="panel lesson-card"><StepHeading icon={<Code2 />} item={item} /><div className="lesson-prose"><p>{problem.prompt}</p><div className="code-tabs"><button className={`code-tab ${codeTab === 'starter' ? 'active' : ''}`} onClick={() => setCodeTab('starter')} data-testid="button-code-starter">Starter</button><button className={`code-tab ${codeTab === 'solution' ? 'active' : ''}`} onClick={() => setCodeTab('solution')} data-testid="button-code-solution">Reference solution</button></div><pre className="code-block"><code>{codeTab === 'starter' ? problem.starterCode : problem.solutionCode}</code></pre><div className="complexity"><span>time · {problem.complexity.time}</span><span>space · {problem.complexity.space}</span></div></div></div>;
}
